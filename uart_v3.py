...
def acquire_and_plot_voltages():
    """Acquire one 20-k-sample burst, update time/FFT plots (dB view), return metrics."""

    print("Waiting for signal…")
    while ser.readline().decode('utf-8', errors='ignore').strip() != "START":
        pass

    # ── 1. Raw ADC burst ────────────────────────────────────────────────────
    data      = ser.read(TOTAL_SAMPLES * 2)             # 40 kB binary
    adc       = np.frombuffer(data, dtype=np.uint16)

    # ── 2. Scale & simple de-noising ───────────────────────────────────────
    v         = adc.astype(np.float32) * 3300 / 4095    # mV
    v_filt    = filter_voltage(v)                       # ±100 mV dead-band
    v_ac      = v_filt - v_filt.mean()                  # remove DC
    x         = v_ac * np.hanning(len(v_ac))            # window

    # ── 3. Time-domain plot ────────────────────────────────────────────────
    line_time.set_data(np.arange(TOTAL_SAMPLES) / SAMPLING_RATE, v_filt)
    ax_time.relim(); ax_time.autoscale_view()

    # ── 4. FFT & spectrum plot (dB) ────────────────────────────────────────
    fft       = np.fft.fft(x)
    mag_lin   = np.abs(fft) / len(x)                    # linear amplitude
    mag_db    = 20*np.log10(mag_lin + 1e-12)            # dB re 1 (arbitrary)

    freqs     = np.fft.fftfreq(len(x), d=1/SAMPLING_RATE)
    half      = len(x) // 2                             # one-sided arrays

    # Optional: normalise so the largest bin is 0 dB
    mag_db   -= mag_db[:half].max()

    line_freq.set_data(freqs[:half], mag_db[:half])
    ax_freq.set_xlim(1, SAMPLING_RATE/2)                # 1 Hz – 5 kHz
    ax_freq.set_xscale('log')
    ax_freq.set_yscale('linear')                        # dB is already log
    ax_freq.set_ylabel('Magnitude (dB)')
    ax_freq.relim(); ax_freq.autoscale_view()

    fig.canvas.draw(); fig.canvas.flush_events(); plt.pause(0.05)

    # ── 5. Feature extraction (use linear magnitude) ───────────────────────
    thr               = 0.5 * mag_lin[:half].max()
    idx               = np.where(mag_lin[:half] >= thr)[0]
    bandwidth         = freqs[idx[-1]]-freqs[idx[0]] if idx.size>1 else 0
    peaks, _          = find_peaks(mag_lin[:half], height=0.1*mag_lin[:half].max())
    dom_harm          = freqs[peaks]

    psd               = np.abs(fft[:half])**2           # power spectrum
    avg_energy        = psd.mean()
    tot_energy        = psd.sum()
    freq_centroid     = (freqs[:half]*mag_lin[:half]).sum() / mag_lin[:half].sum()
    old_energy        = mag_lin[:half].sum()

    return (v_filt,
            freqs[np.argmax(mag_lin[:half])],  # peak frequency (Hz)
            avg_energy, tot_energy, old_energy,
            freq_centroid, bandwidth, dom_harm)
